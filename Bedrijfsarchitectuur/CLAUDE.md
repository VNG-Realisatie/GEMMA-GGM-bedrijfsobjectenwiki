...
- [BO8] **Specialisatie met of zonder eigen pagina bij hernoeming** — Bij hernoeming wordt de specifieke term binnen de hernoemde BO een specialisatie zonder eigen pagina (`bo_subtypes`) OF een specialisatie MET eigen pagina (ALLEEN ALS de term zelf de 6 criteria haalt).
...
- [BO12] **BO-veldnamen en prefixen** — Veldnamen: `bo_definitie`, `bo_toelichting`, `bo_relaties`, `bo_synoniemen` (andere namen voor hetzelfde concept), `bo_homoniemen` (andere concepten met dezelfde GGM-naam). `bo_subtypes`: zie [BO15] en [BO16] — de veldnaam blijft ongewijzigd, maar de body-sectie en het leesbare begrip heten sinds besluit 2026-09-24 "specialisatie zonder eigen pagina", niet meer "subtype". `bedrijfsprocessen` en `bedrijfsfuncties` behouden hun naam; sinds besluit 2026-09-23 zijn dit wiki-links naar eigen elementpagina's in `Wiki/Bedrijfsprocessen/`/`Wiki/Bedrijfsfuncties/` (zie §7, `templates/element.md`), geen vrije tekst meer — vrije-tekstvermeldingen van vóór dit besluit zijn migratie-achterstand, gesignaleerd door `/lint`. Prefix `bo_` = wiki-eigen BO-model; `ggm_*` = GGM-bron; `ggm_gemma_*` = GGM-GEMMA-referentie. De export leest de `bo_`-velden.
...
- [BO16] **Term "subtype" vervangen door "specialisatie (zonder eigen pagina)"** — Besluit 2026-09-24: de term "subtype" wordt NIET meer gebruikt in body-secties, skills of documentatie (matcht ArchiMate, dat alleen "specialization" kent). Eén `## Specialisaties`-sectie voor alle specialisaties van een BO: rij is een `[[wiki-link]]` als de specialisatie een eigen BO-pagina heeft, platte tekst als niet (dan vastgelegd in `bo_subtypes`, ongewijzigde veldnaam — zie [BO15]). Aanleiding: de oude, losse `## Specialisaties`- (eigen pagina) en `## Subtypes`-secties (geen eigen pagina) hadden een gedeelde tabelkolomnaam "Subtype", wat bij herhaling tot foutieve dubbele secties leidde (content zonder eigen pagina onterecht in de Specialisaties-tabel, plus een lege Subtypes-stub) — gevonden in 25 BO-pagina's. Zie `templates/element.md` §Specialisaties format voor het volledige format, incl. de aanpak bij twee onafhankelijke classificatie-assen (subkopjes `### Naar {as}`).
