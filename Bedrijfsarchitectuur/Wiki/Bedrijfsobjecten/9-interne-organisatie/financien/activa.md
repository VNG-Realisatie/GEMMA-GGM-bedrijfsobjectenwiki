---
type: bedrijfsobject
naam: Activa
domein: [Financien]
archimate_type: "business-object"
grondslag: "ggm-entiteit"
ggm_entiteit: "Activa"
ggm_guid: EAID_7B8D0A8F_07BF_42bf_90B3_28A72BD4401A
ggm_uml_type: Class
ggm_beleidsdomein: "Financien"
ggm_taakveld: "9 Interne Organisatie"
ggm_diagram: [Financien Verplichtingen en Facturen]
ggm_diagram_ids: [EAID_0723EB5C_4A2C_44d4_B15B_37AC71B5D711]
ggm_definitie: "Bezittingen van een onderneming op een boekhoudkundige balans"
ggm_toelichting: ""
ggm_synoniemen: ""
ggm_herkomst: ""
ggm_gemma_naam: "Activa"
ggm_gemma_guid: "e83e86ad-4769-41ce-91c7-675e93441074"
ggm_gemma_definitie: "Bezittingen van een onderneming op een boekhoudkundige balans"
ggm_gemma_toelichting: ""
ggm_gemma_synoniemen: ""
ggm_gemma_type: "business-object"
ggm_gemma_url: "https://gemmaonline.nl/index.php/GEMMA/id-e83e86ad-4769-41ce-91c7-675e93441074"
ggm_gemma_bron: ""
ggm_gemma_alternate_name: ""
gemma_definitie: "Bezittingen van de gemeente zoals vastgelegd op de boekhoudkundige balans."
definitie: Bezittingen van een onderneming op een boekhoudkundige balans
bedrijfsprocessen: [Activabeheer, Jaarrekening]
bronnen: [Wiki/Bronsamenvattingen/Financien/raadgever-inkomstenbronnen, Wiki/Bronsamenvattingen/Financien/raadgever-gemeentebegroting, Wiki/Bronsamenvattingen/Financien/raadgever-financiele-verordening, Wiki/Bronsamenvattingen/Financien/raadgever-financiele-conditie]
bedrijfsfuncties: [Financieel beheer]
status: concept
---

# Activa

Bezittingen van een onderneming op een boekhoudkundige balans. In de gemeentelijke context: vastgoed, infrastructuur, voertuigen en andere duurzame bezittingen.

## GGM-bron

> Bezittingen van een onderneming op een boekhoudkundige balans

- **Entiteit:** Activa
- **Beleidsdomein:** Financien (taakveld 9 — Interne Organisatie)
- **Attributen:** naam, omschrijving

## Bedrijfsprocessen

- **Activabeheer** — registratie, waardering en afschrijving van bezittingen
- **Jaarrekening** — verantwoording op de balans

## Bedrijfsfuncties

- **Financieel beheer** — vermogensbeheer

## Relaties

- Wordt gewaardeerd en afgeschreven via de [[Wiki/Bedrijfsobjecten/9-interne-organisatie/financien/begroting|begroting]]
- Gekoppeld aan [[Wiki/Bedrijfsobjecten/9-interne-organisatie/financien/kostenplaats|kostenplaats]]en voor kostentoerekening
