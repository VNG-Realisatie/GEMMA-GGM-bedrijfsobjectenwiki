## [2026-09-24] structuur | Vervolg: "subtype" ook opgeschoond in Onderwerpoverzichten en export-tooling

- **Aanleiding:** vraag "zijn er nog bijwerkingen op entiteitendekking?" na de BO-pagina-migratie hierboven. `entiteitendekking.py`/`entiteitendekking_sync_bo.py` bleken zelf niet te breken (ze parsen geen `## Subtypes`-body-headers, alleen `bo_subtypes`-frontmatter en de al-consistente `specialisatie van {BO}`-labels), maar drie andere plekken bleken nog "subtype" te gebruiken:
  1. **`tools/entiteitendekking.py` — `BEGRIP_TYPE_MAP`**: had een key `'subtype'` (leest de `Type`-kolom van onderwerpoverzicht-begrippentabellen). Hernoemd naar `'specialisatie'`.
  2. **`Wiki/Onderwerpoverzichten/*.md` — begrippentabellen**: "subtype" kwam voor in drie rollen: (a) 30 rijen met letterlijk `subtype` als `Type`-kolomwaarde (`basisregistraties.md` 8×, `belastingen.md` 17×, `economie.md` 4×, `milieu.md` 1×) — code-relevant, gekoppeld aan punt 1; (b) 16 rijen in `sociaal-domein.md` met `subtype` in de `BO?`-kolom (hoort daar niet, geen `❌`, dus toch al genegeerd door de code — genormaliseerd naar `—`); (c) ~90 vrije-tekst "Reden"-vermeldingen ("Subtype van [[BO]]", "te granulair; subtype klacht", etc.) over 25 bestanden — puur beschrijvend, verbatim overgenomen in de gegenereerde `Wiki/Analyses/entiteitendekking/{taakveld}.md`-rapporten. Alle drie categorieën hernoemd naar "specialisatie (van)".
  3. **`tools/export_ggm_csv.py`**: CSV-kolom `GEMMA-subtypes` (geëxporteerd, mogelijk extern geconsumeerd) hernoemd naar `GEMMA-specialisaties`. De losstaande `bo_subtypes`-kolom in dezelfde export blijft ongewijzigd (veldnaam, geen leesterm).
- **Bewust niet aangepast:** `bo_subtypes`-veldverwijzingen in proza (bijv. `informatiesamenleving.md` regel 76) — dat is de frontmatter-veldnaam zelf, geen methodologieterm.
- **Pre-existente bevinding, niet gefixt (buiten scope van deze opschoning):** `milieu.md` regel 89 markeert Sluis als `✅` (BO) terwijl de bronpagina [[Wiki/Bedrijfsobjecten/8-volkshuisvesting-leefomgeving-en-stedelijke-vernieuwing/beheer-openbare-ruimte/kunstwerk|Kunstwerk]] Sluis juist als specialisatie-zonder-eigen-pagina voert (`bo_subtypes`, geen wiki-link) — een inhoudelijke inconsistentie, los van de terminologie. En `sociaal-domein.md`'s kruisverwijzing naar GGM-terugmelding #44 blijkt een ander onderwerp (Milieu/Ontheffing) te betreffen — stale nummerverwijzing, ook niet gefixt.
- **Geverifieerd:** `python3 tools/lint_checks.py` draait foutloos; `entiteitendekking.get_begrip_index()` classificeert de hernoemde rijen (bijv. "leges", "winkelgebied") correct als `type: specialisatie`; rijen die al buiten de index vielen (geen `❌` in BO?-kolom, bijv. "sluis", "buurtcentrum") blijven daar terecht buiten — geen gedragsverandering, alleen naamgeving.

## [2026-09-24] structuur | Term "subtype" vervangen door "specialisatie (zonder eigen pagina)" — wiki-breed

- **Aanleiding:** bij het verrijken van BO Woning (zie ingest-entry hieronder) bleek de body-structuur inconsistent: een `## Specialisaties`-sectie met content die eigenlijk `bo_subtypes` was (geen eigen pagina), plus een aparte, deels dubbele `## Subtypes`-sectie. Root cause: in `templates/element.md` en `write-element.md` heette de tabelkolom bij `## Specialisaties` (voor children MET eigen pagina) toevallig ook "Subtype" — die naamcollisie leidde er bij herhaling toe dat specialisaties-zonder-pagina in de verkeerde sectie belandden. Gevonden in **25 BO-pagina's**, verspreid over 9 taakvelden — systemisch, niet incidenteel.
- **Besluit (met gebruiker, [BO16]):** term "subtype" laten vallen, matcht ArchiMate (dat alleen "specialization" kent). Eén `## Specialisaties`-sectie voor alle gevallen: rij is `[[wiki-link]]` bij een eigen BO-pagina, platte tekst zonder. `bo_subtypes`-veldnaam blijft ongewijzigd (interne datasleutel, geen leesterm) — [BO15] blijft van kracht.
- **Regeldocumentatie bijgewerkt:** `templates/element.md` (Specialisaties-format samengevoegd met de oude Subtypes-en-GGM-componenten-format, Overzicht-hiërarchiesecties-tabel, `bo_subtypes`-veldcommentaar), `.claude/commands/write-element.md` (Stap 6b/6c samengevoegd tot één Stap 6b), `.claude/commands/assess-element.md` (Stap 4- en Stap 8-beslistabellen), `.claude/commands/lint.md` (bevindingnamen), `CLAUDE.md` ([BO8], [BO12], nieuw [BO15]/[BO16]), `documentatie.md`.
- **`tools/lint_checks.py` aangepast:** `check_subtypes()` keek naar een `## Subtypes`-sectie en matchte alleen `**vetgedrukte bullets**` — dat zou na de hernoeming alle 118 `bo_subtypes`-pagina's als "sectie ontbreekt" hebben gemeld. Nu: kijkt naar `## Specialisaties`, herkent zowel de oude bullet-vorm als tabelrijen, en negeert rijen met een `[[wiki-link]]` (die horen bij `bo_relaties`, niet `bo_subtypes`). Rapportlabels hernoemd (regel 988-990).
- **Contentmigratie:** 42 pagina's met alleen `## Subtypes` → header hernoemd naar `## Specialisaties` (puur mechanisch, geen inhoudswijziging). 25 pagina's met beide secties → gereconcilieerd: in 23 gevallen bleek de content identiek of was de Specialisaties-tabel al een superset, dus is de dubbele `## Subtypes`-sectie verwijderd; 2 gevallen kregen inhoudelijke aandacht:
  - [[Wiki/Bedrijfsobjecten/4-onderwijs/onderwijs/kinderopvangvoorziening|Kinderopvangvoorziening]] — "Tussenschoolse opvang" stond alleen in de (verwijderde) Subtypes-bullet, niet in `bo_subtypes` frontmatter noch in de Specialisaties-tabel; toegevoegd aan beide.
  - [[Wiki/Bedrijfsobjecten/99-kern/nhr/vestiging|Vestiging]] — enige pagina met een **echte** mix: Horecabedrijf/Hotel zijn specialisaties MET eigen pagina (al correct in `bo_relaties`), Commerciële/Niet-commerciële vestiging zijn specialisaties ZONDER pagina (stonden alleen in de body, niet in frontmatter — `bo_subtypes` aangevuld, gekoppeld aan GGM-attribuut `commercieleVestiging`). Samengevoegd tot één tabel.
  - 14 pagina's met een correcte, losstaande `## Specialisaties` (eigen-pagina-only, geen dubbele sectie) hadden nog de oude kolomnaam "Subtype" — hernoemd naar "Specialisatie".
- **Bewust niet gedaan:** losse tekstuele vermeldingen van het woord "subtype" in beschrijvende proza (buiten headers/kolomnamen) zijn niet wiki-breed doorzocht en vervangen — te veel valse-positieven-risico (bijv. citaten uit bronnen) voor de gevonden meerwaarde. `/lint` surfacet twee resterende, pre-existente databevindingen die geen gevolg zijn van deze migratie (frontmatter/body-mismatches bij Kunstwerk en Verzekering, `bo_subtypes` zonder `## Specialisaties`-sectie bij 6 pagina's) — voorgelegd, niet blind gefixt.
- **Geverifieerd:** `python3 tools/lint_checks.py` draait foutloos na de migratie; geen dubbele of lege Specialisaties/Subtypes-secties meer wiki-breed.

## [2026-09-24] ingest | GEBORA/IMWO verwerkt als verificatiebron voor Vastgoed en Wonen

- **Aanleiding:** `ToDo/Op te halen bronnen.md` bevatte 3 URL's. De VNG-bron over subsidieregeling politieke partijen bleek al volledig verwerkt (stale regel — `ingest-backlog.md` had 'm al afgevinkt, met BO Partijsubsidie). De overige twee (GEBORA-PDF, IMWO-HTML) zijn opgehaald, geconverteerd en opgeslagen in `Sources/Standaarden/` (gebruiker koos expliciet voor Standaarden i.p.v. Vastgoed/Wonen/nieuw onderwerp als opslaglocatie).
- **GEBORA** (digiGO, Gebouwde Omgeving Referentie Architectuur, v1.0): 12 informatiedomeinen, ~35 bedrijfsobjecten. **IMWO** (digiGO Informatiemodel Woongebouwen, v1.0): expliciete uitwerking van GEBORA-domein Bouwwerk, ~200 formeel gedefinieerde objecttypen (Top/Bouwwerken/Bouwcomponenten/Woonobjecten/Woonobjectruimten/Installaties/Juridische objecten).
- **Scope-afbakening (met gebruiker afgestemd):** alleen het gemeentelijk-relevante deel beoordeeld (vastgoedbeheer, woning/woongebouw). Bouwsector-/ketenpartnerprocessen (Product, Materiaal, Materieel, Personeel, Werkcontract, Order, Transport, Logistiek; IMWO-Bouwcomponenten op onderdeelniveau, Woonobjectruimten op kamerniveau, Installaties, Slimme woning) buiten scope per [WC5].
- **Resultaat: 0 nieuwe BO's.** Beide bronnen bevestigen grotendeels het bestaande model:
  - Vastgoed-domein: Asset/Object → duplicaat [[Wiki/Bedrijfsobjecten/9-interne-organisatie/vastgoed/vastgoedobject|Vastgoedobject]]; Assetplan → duplicaat [[Wiki/Bedrijfsobjecten/9-interne-organisatie/vastgoed/mjop|MJOP]]; Huur/Verhuur → duplicaat [[Wiki/Bedrijfsobjecten/9-interne-organisatie/vastgoed/vastgoedcontract|Vastgoedcontract]]; Vergunning → duplicaat [[Wiki/Bedrijfsobjecten/10-dienstverlening/dienstverlening/vergunningen-en-ontheffingen|Vergunningen en ontheffingen]]. Assetinitiatief, Instandhouding/Onderhoud/Conditie, Koop/Verkoop: procesmatig, halen de BO-criteria niet zelfstandig. Zone/Bestemming/Gebied: horen bij Omgevingswet-domein.
  - Wonen-domein: WoonGebouw → duplicaat [[Wiki/Bedrijfsobjecten/99-kern/bag/pand|Pand]] (BAG); BouwComplex → geen zelfstandig object (valt samen met [[Wiki/Bedrijfsobjecten/8-volkshuisvesting-leefomgeving-en-stedelijke-vernieuwing/bouwen-en-wonen/woningbouwplan|Woningbouwplan]] of een verzameling Panden).
- **Wel verrijkt:** BO [[Wiki/Bedrijfsobjecten/8-volkshuisvesting-leefomgeving-en-stedelijke-vernieuwing/bouwen-en-wonen/woning|Woning]] — 11 architecturale bouwvorm-`bo_subtypes` uit IMWO toegevoegd (nieuwe classificatie-as naast het bestaande marktsegment sociale huur/middenhuur/koop/student), plus een `bo_relaties`-koppeling naar [[Wiki/Bedrijfsobjecten/99-kern/bag/pand|Pand]] (ontbrak eerder — GGM-entiteit Gebouw heeft geen relatie naar Pand).
- **2 nieuwe GGM-terugmeldingen:** #128 (enumeratie `soortWoonobject` in Model BAG/RSGBPlus heeft geen ingevulde literalen — IMWO levert de concrete bouwvorm-typen), #129 (GGM-entiteit Gebouw mist relatie naar Pand, terwijl IMWO deze laag als WoonGebouw expliciet modelleert).
- **Bijgewerkt:** `Wiki/Onderwerpoverzichten/vastgoed.md` en `wonen.md` (begrippentabellen, bronnen_count/begrippen_count, verificatie-update), `Wiki/Analyses/ggm-terugmeldingen.md`, `Wiki/index.md`, `ToDo/ingest-backlog.md` (nieuwe sectie afgevinkt), `ToDo/Op te halen bronnen.md` (geleegd).

## [2026-09-24] structuur | Elementtype-criteria geconsolideerd, Wiki/GEMMA/ opgeheven

- **Aanleiding:** `ToDo/assesment-criteria.md` signaleerde twee knelpunten: (1) de regels om een begrip als bedrijfsobject/actor/rol/bedrijfsfunctie/-proces te classificeren stonden verspreid over `CLAUDE.md`, `assess-element.md` en twee losse `Wiki/GEMMA/`-pagina's; (2) `templates/element.md` mixte generieke en BO-specifieke velden/secties zonder duidelijke scheiding. Bij het uitzoeken bleek `Wiki/GEMMA/` zelf ook een onduidelijke plek: naast de twee criteria-pagina's stond er een bronsamenvatting (`gemma-bedrijfsobjecten-en-ggm.md`) die daar per abuis terecht was gekomen (was oorspronkelijk aangemaakt onder `Wiki/Bronsamenvattingen/GEMMA/`, zie regel 1934 hieronder).
- **Kernbeslissing:** classificatiecriteria zijn regels die een skill uitvoert, geen wiki-inhoud (bestaand principe uit `CLAUDE.md` §6: "Regels die één skill uitvoert staan in die skill"). Ze horen dus niet in `Wiki/` maar bij het proces-referentiemateriaal.
- **Nieuw:** `templates/elementtype-criteria.md` — één bestand, generiek bovenaan (de begripstype-tabel, alle 10 typen + routering + stopregel), daarna drie symmetrische typespecifieke secties (Bedrijfsobject, Actor en rol, Bedrijfsfunctie en bedrijfsproces), elk met definitie(s) → criteria/diagnostische vragen → beslisvraag → scope. Consolideert de oude `Wiki/GEMMA/actoren-en-rollen.md` en `functies-en-processen.md` plus de tot nu toe inline BO-criteria/begripstype-tabel uit `assess-element.md`.
- **`Wiki/GEMMA/` opgeheven:** `gemma-bedrijfsobjecten-en-ggm.md` terugverplaatst naar `Wiki/Bronsamenvattingen/GEMMA/` (zijn oorspronkelijke, correcte plek — gewoon een bronsamenvatting, geen aparte behandeling nodig).
- **`templates/element.md` herstructureerd:** frontmatter en Secties-lijst elk opgesplitst in "generiek (alle typen)" en "alleen bedrijfsobject". `grondslag` en de meeste `bo_*`-velden blijken generiek (bevestigd door `write-element.md` Stap 11/12); alleen `bo_subtypes`/`bo_via_kandidaten` en de hiërarchie-/herkomstsecties (Generalisatie, Specialisaties, Subtypes, GGM-componenten, Naamkeuze, GGM-duplicaten, Afleiding, Procesbron, Juridische bron) zijn BO-only. De "Criteria-toetsing"-sectie is nu expliciet generiek en verwijst per type naar de juiste sectie in `templates/elementtype-criteria.md`.
- **Doorverwezen:** `assess-element.md` (Stap 2/2b/2c/7/7b), `write-element.md` (Stap 11/12), `audit-element.md`, `entiteitendekking.md`, `CLAUDE.md` (§4, §7-directorystructuur), `Wiki/index.md` (Actoren/Rollen/Bedrijfsfuncties/Bedrijfsprocessen-secties en de bronsamenvatting-link) — alle voormalige `Wiki/GEMMA/...`-links vervangen door verwijzingen naar `templates/elementtype-criteria.md`. `Wiki/log.md`-historie (chronologisch) en de precedentnoot in `ToDo/Structuur en herleidbaarheid.md` blijven ongewijzigd resp. met een korte voetnoot bijgewerkt.
- **Anti-patronen [BO1]-[BO3] blijven in `CLAUDE.md`:** actief ID-gerefereerd vanuit `assess-element.md`, `ingest.md`, `write-element.md` en `lint.md` — niet verplaatst, om de bestaande ID-verwijzingen niet te breken.
