---
type: bronsamenvatting
bron: "[Sources/Onderwerpen/Ruimte Wonen en Mobiliteit/historische-schepen-utrecht-2015.md](Sources/Onderwerpen/Ruimte Wonen en Mobiliteit/historische-schepen-utrecht-2015.md)"
titel: Behouden van historische schepen in Utrecht (december 2015)
domein: [Wonen]
datum_ingest: 2026-06-21
---

# Behouden van historische schepen in Utrecht (december 2015)

## Samenvatting

Aanvulling op het woonbotenbeleid uit 2007. De gemeente wijst drie zones aan waar bij voorkeur historische woonboten of bedrijfsvaartuigen mogen liggen: Keulsekade (Merwedekanaal), Vechtdijk (Vecht) en Oosterkade (Vaartsche Rijn).

### Definitie historisch schip

Een [[Woonboot]] of bedrijfsvaartuig is historisch als het minimaal 50 jaar oud is en de originele romp is ontworpen om te varen. De opbouw moet nog duidelijk oorspronkelijke kenmerken bezitten. Een "schark" — schip met originele romp en (deels) vervangen houten opbouw — wordt ook als historisch aangemerkt. Er zijn circa 60 historische woonboten en scharken in Utrecht (dalend aantal).

### Vervangingsregeling

In de aangewezen zones geldt:
- Historische boten worden altijd vervangen door historische boten
- Bij schepen >18 m: vervanging door historisch schip met dezelfde of kortere lengte
- Bij schepen <18 m: vervanging door historisch schip van maximaal 18 meter
- Eenmalige lengtemarge van 10% bij vervanging (indien ligplaatsomvang het toelaat)
- Niet-historische boten mogen op vrijwillige basis vervangen worden door historische boten

### Vormgevingseisen

Ontleend aan de Amsterdamse Nota welstand op het water:
- Aan- en opbouwen afgestemd op oorspronkelijke vormgeving
- Luikenkap/roef maximaal 2,70 m boven waterlijn
- Buitenmaten casco mogen niet worden overschreden
- Kleur- en materiaalgebruik afgestemd op het origineel

### De drie zones

- **Keulsekade**: Merwedekanaal tussen Spinozabrug en sluizencomplex, oostzijde. 10 historische woonboten, 2 woonarken, 1 modern jacht.
- **Vechtdijk**: langs de Vecht, tegenover Vechtplantsoen. 5 historische woonboten.
- **Oosterkade**: Vaartsche Rijn, historische kade. 2 industriële schepen (tijdelijk verwijderd voor station Vaartsche Rijn).

## Kernbegrippen

- **Historisch schip** — woonboot of bedrijfsvaartuig ≥50 jaar oud met originele romp en kenmerken; classificatie van [[Woonboot]]
- **Schark** — historisch schip met originele romp en (deels) vervangen houten opbouw; subtype van historisch schip
- **Zone historische boten** — aangewezen stadsdeel met beschermingsregime voor historische vaartuigen
- **Vervangingsregeling** — regels voor wat een historisch schip mag vervangen (type, lengte, lengtemarge)

## Relevantie voor bedrijfsarchitectuur

Introduceert "historisch schip" als classificatie van [[Woonboot]], niet als zelfstandig objecttype. De drie zones zijn een ruimtelijk beleidsinstrument (classificatie van gebieden), geen zelfstandig registreerbaar object. De vervangingsregeling is een beleidsregel die op de ligplaatsvergunning wordt toegepast.

## Citaten

> "Zo'n boot moet minimaal 50 jaar oud zijn. Als de originele romp van een boot of schip ontworpen is om te varen en de opbouw nog duidelijk oorspronkelijke kenmerken bezit, dan spreken we van een historische woonboot."
— §1.1 Aanleiding en uitgangspunten

> "In deze zones worden historische boten altijd vervangen door historische boten."
— §1.2 Doel en beoogde resultaten
