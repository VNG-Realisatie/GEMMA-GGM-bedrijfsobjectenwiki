---
type: onderwerp
naam: Informatiebeheer
status: in-behandeling
verwerkingsdatum: 2026-09-23
bronnen_count: 1
begrippen_count: 6
bo_count: 3
---

# Informatiebeheer

Archivering en gegevensbeheer bij de gemeente: hoe informatie de levenscyclus doorloopt van actief gebruik tot archivering en overbrenging, en wie daarvoor verantwoordelijk is. Dit onderwerp overlapt met Dienstverlening (Document/Zaak), Cultuur (Archiefstuk/erfgoed) en Bestuur (Archivaris), en bundelt hun gedeelde archivale kant.

## Begrippen

| Begrip | Begripstype | Omschrijving | BO? | Data-object | Reden | Voorbeelden | GGM |
|---|---|---|---|---|---|---|---|
| [[Wiki/Bedrijfsobjecten/10-dienstverlening/dienstverlening/document\|Document]] | object | Geheel van gegevens met eigen identiteit, ontvangen of opgemaakt bij taakuitvoering (actieve fase) | ✅ | ja | 6/6 criteria, GGM exact | Brief, e-mail, besluit | ja |
| [[Wiki/Bedrijfsobjecten/10-dienstverlening/dienstverlening/informatieobject\|Informatieobject]] | object | Document dat de archiveringsfase is ingegaan: geselecteerd, gewaardeerd en formeel opgenomen in het archiefsysteem | ✅ | ja | 6/6 criteria, GGM-hiaat (archivale beheerfase tussen Document en Archiefstuk ontbreekt) | Gearchiveerd zaakdossier in e-depot | nee |
| [[Wiki/Bedrijfsobjecten/5-sport-cultuur-en-recreatie/erfgoed/archiefstuk\|Archiefstuk]] | object | Informatieobject na overbrenging naar een archiefbewaarplaats | ✅ | ja | 6/6 criteria, GGM exact | Overgebracht raadsarchief 1980-2000 | ja |
| [[Wiki/Rollen/archiefvormer\|Archiefvormer]] | rol | Partij die verantwoordelijk blijft voor (uitgeplaatste) archiefbescheiden, ook na fysieke overdracht aan een archiefdienst | ❌ | nee | Rol (verantwoordelijkheid), geen eigen gegevensregistratie; vastgelegd als rol-pagina, geen BO | Gemeente als archiefvormer van overgebracht raadsarchief | nee |
| [[Wiki/Rollen/archivaris\|Archivaris]] | rol | Door college of GR-bestuur aangewezen functionaris die overgebrachte archiefbescheiden beheert (art. 32 Archiefwet) | ❌ | nee | Rol (wettelijke functie), geen eigen gegevensregistratie; vastgelegd als rol-pagina, geen BO | Gemeentearchivaris | nee |
| [[Wiki/Rollen/archiefinspecteur\|Archiefinspecteur]] | rol | Functionaris die toezicht houdt op de archiefvorming bij gemeenten en gemeenschappelijke regelingen | ❌ | nee | Rol (toezichthoudende functie), geen eigen gegevensregistratie; vastgelegd als rol-pagina, geen BO | Provinciaal/regionaal archiefinspecteur | nee |

**Drie fasen, geen synoniemen:** Document (actief) → Informatieobject (gearchiveerd) → Archiefstuk (overgebracht) vormen één levenslijn, zie [[Wiki/Bronsamenvattingen/informatiebeheer/overheidsinformatiemodel|Overheidsinformatiemodel]]. Het GGM dekt de eerste en de laatste fase (Document, Archiefstuk) maar niet de tussenliggende archivale beheerfase — vandaar het GGM-hiaat bij Informatieobject.

## Verwerkte bronnen

- [[Wiki/Bronsamenvattingen/informatiebeheer/overheidsinformatiemodel|Overheidsinformatiemodel (Nationaal Archief)]] — conceptueel model archivering/gegevensmanagement; grondslag voor de Document → Informatieobject → Archiefstuk-levenslijn

## Nog te verwerken bronnen

Geen bekende onverwerkte bronnen in `Sources/Onderwerpen/informatiebeheer/`.

**2026-09-23:** vier bronnen over Woo-actieve openbaarmaking bleken in deze map te staan, maar horen inhoudelijk bij [[Wiki/Onderwerpoverzichten/informatiesamenleving|Informatiesamenleving]] (waar Woo al is uitgewerkt: Woo-verzoek, Klacht, Woo-contactpersoon). Verplaatst; zie de "Beoordeling"-sectie aldaar.

## Openstaande vragen of hiaten

Geen.

## Terugmeldingen richting GGM

Zie [[Wiki/Analyses/ggm-terugmeldingen|GGM-terugmeldingen]] — #98: Informatieobject ontbreekt als objecttype tussen Document en Archiefstuk.
