...
toevoegen. Geldt voor [[project_redactie-gemmaonline-toegang]] / werk binnen `GEMMA online/`.

## lint-haiku-verificatie

Bij `/lint`-runs op de Bedrijfsarchitectuur-wiki (draait bewust op Haiku, zie [[project_element-schema]]) altijd een steekproef van de gemelde bevindingen zelf verifiëren (grep/Read/eigen script) vóór ze in het eindrapport komen — dit geldt voor élke categorie, ook pure telbare/structurele claims, niet alleen semantische.
